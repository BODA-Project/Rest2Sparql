...
public class QueryFactoryTest {
    ...
    public void testBuildObservationQuery_3() throws Exception {
        ...
        String exp = "PREFIX qb: <http://purl.org/linked-data/cube#> PREFIX code: <http://code-research.eu/resource/> PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX xsd: <http://www.w3.org/2001/XMLSchema#> PREFIX prov: <http://www.w3.org/ns/prov#> SELECT ?D_NAME_0 ?E_NAME_0 ?L_NAME_0 ?E_NAME_1 (SAMPLE(?D_NAME_1) AS ?E_NAME_1_AGG) (SAMPLE(?L_NAME_1) AS ?L_NAME_1_AGG) ?M_NAME_2 ?V_NAME_2 WHERE { ?OBS_NAME a qb:Observation. ?OBS_NAME ?D_NAME_0 ?E_NAME_0. ?E_NAME_0 <http://www.w3.org/2000/01/rdf-schema#label> ?L_NAME_0. ?OBS_NAME ?D_NAME_1 ?E_NAME_1. ?E_NAME_1 <http://www.w3.org/2000/01/rdf-schema#label> ?L_NAME_1. ?OBS_NAME ?M_NAME_2 ?V_NAME_2. ?CUBE_NAME prov:wasGeneratedBy ?import. ?import prov:wasStartedBy ?importer. ?importer rdfs:label \"8023903\"^^xsd:string. ?OBS_NAME qb:dataSet ?C_NAME_3. FILTER (?D_NAME_0 = <http://dbpedia.org/resource/Timestamp>). FILTER (?D_NAME_1 = <http://dbpedia.org/resource/River>). FILTER (?M_NAME_2 = <http://dbpedia.org/resource/Water_level>). FILTER (?V_NAME_2 > \"1000\"^^xsd:integer). FILTER (?C_NAME_3 = <http://code-research.eu/resource/Dataset-f744647d-e493-4640-9bd6-2080779a5e77>). } GROUP BY ?E_NAME_1 ORDER BY ?D_NAME_0 LIMIT 100";
        ...
        Measure m1 = new Measure("http://dbpedia.org/resource/Water_level", new Parameters(true, 0, false, AggregateFunction.NONE, Relation.BIGGER, "\"1000\"^^xsd:integer", AggregateFunction.NONE, null, null));
        objects.add(d1);
        objects.add(d2);
        objects.add(m1);
        ...
        String res = QueryFactory.buildObservationQuery(new QueryDescriptor(objects, 100, "8023903", "7fb2f0dd7d608ea6b82eaa9b6e38aa80e1b266d8f8610a2c4c2671368df2b7bc"));
        Assert.assertEquals(exp, res);
    ...
    public void testBuildObservationQuery_4() throws Exception {
        ...
        String exp = "PREFIX qb: <http://purl.org/linked-data/cube#> PREFIX code: <http://code-research.eu/resource/> PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#> PREFIX xsd: <http://www.w3.org/2001/XMLSchema#> PREFIX prov: <http://www.w3.org/ns/prov#> SELECT ?E_NAME_2 (SAMPLE(?D_NAME_2) AS ?E_NAME_2_AGG) (SAMPLE(?L_NAME_2) AS ?L_NAME_2_AGG) (SAMPLE(?V_NAME_3) AS ?V_NAME_AGG) WHERE { ?OBS_NAME a qb:Observation. ?OBS_NAME ?D_NAME_1 ?E_NAME_1. ?E_NAME_1 <http://www.w3.org/2000/01/rdf-schema#label> ?L_NAME_1. ?OBS_NAME ?D_NAME_2 ?E_NAME_2. ?E_NAME_2 <http://www.w3.org/2000/01/rdf-schema#label> ?L_NAME_2. ?OBS_NAME ?M_NAME_3 ?V_NAME_3. ?CUBE_NAME prov:wasGeneratedBy ?import. ?import prov:wasStartedBy ?importer. ?importer rdfs:label \"8023903\"^^xsd:string. ?OBS_NAME qb:dataSet ?C_NAME_0. FILTER (?C_NAME_0 = <http://code-research.eu/resource/Dataset-f744647d-e493-4640-9bd6-2080779a5e77>). FILTER (?D_NAME_1 = <http://dbpedia.org/resource/Timestamp>). FILTER (?D_NAME_2 = <http://dbpedia.org/resource/River>). FILTER (?M_NAME_3 = <http://dbpedia.org/resource/Water_level>). } GROUP BY ?E_NAME_2 LIMIT 100";
