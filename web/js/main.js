...
var GET_HASH_URL = "./backend?func=<getHash>&id=<__id__>";
var EXECUTE_URL = "./backend?func=<execute>&c=<__cube__>,select=<false>&id=<__id__>&hash=<__hash__>";
//var EXECUTE_URL = "func=<execute>&c=<__cube__>,select=<false>&id=<__id__>&hash=<__hash__>";
...
//    $('[data-toggle="tooltip"]').tooltip();
    ...
    showLoadingScreen(""); // TODO hier leer aber später "loading..."
...
function popupLogin() {
    ...
    var modal = $(MODAL_LOGIN_TEMPLATE);
    $("body").append(modal);
...
function loginUser(id) {
    var urlHash = GET_HASH_URL.replace("__id__", id);
    ...
    requestHash.done(function (hash) {
        ...
        alert(id + ", HASH = " + hash); // TEST

        // Also check if given user has any cubes
        var urlCubes = CUBE_URL.replace("__id__", id);
        ...
        requestCubes.done(function (content) {
            ...
            if (results.length === 0) {
                ...
                HASH = hash;
                $.cookie('ID', id);
//                $.cookie('ID', id, {expires: 7}); // TODO: option "remember me" -> sonst nur session-cookie
...
//    var request = $.ajax({
//        url: "./backend",
//        type: "POST", // TODO: post statt get, und dann inhalt als "data", aber geht das noch mit restAPI ?
//        data: {"mainUrl": url},
//        headers: {
//            accept: "application/sparql-results+json"
//        }
...
// TODO: guided forward to next panel + info panels below!!!
...
//                        console.log(dimensionName, entitiyList[dimensionName])
                        ...
                        $.each(entitiyList[dimensionName], function (index, entity) {
                            ...
                            var label = $('<label class="btn btn-default btn-xs ' + (entitiyList[dimensionName][entity.entityName] ? 'active' : '') + '" title="' + entity.label + '"></label>');
...
function addInterfaceListeners() {
    ...
    // Top bar
    $("#id_changeUserButton").on('click', logoutUser);
